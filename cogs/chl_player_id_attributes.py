...
class chl_player_id_attributes(commands.Cog):
    def __init__(self, client: commands.Bot):
        self.client = client
    @app_commands.command(name="chl_player_id_attributes", description="Look up a college hockey player using a player {id}")
    async def chl_player_id_attributes(self, interaction: discord.Interaction, id: str):
        try:
            data = api_requests.GetCollegeHockeyPlayer_id(id)
            if data == False:
...
async def setup(client: commands.Bot):
    await client.add_cog(chl_player_id_attributes(client))
